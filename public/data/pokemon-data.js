...
const abilityResourceCache = new Map();
...
async function fetchAbilityResource(identifier) {
  if (!identifier) return null;
  const key = String(identifier).toLowerCase();
  if (abilityResourceCache.has(key)) {
    return abilityResourceCache.get(key);
  }
  try {
    const data = await fetchJson(`${API_BASE_URL}/ability/${key}`);
    abilityResourceCache.set(key, data);
    return data;
  } catch (error) {
    abilityResourceCache.set(key, null);
    console.warn(`Kon ability ${identifier} niet laden`, error);
    return null;
  }
}
...
function extractAbilityEffect(abilityData) {
  if (!abilityData) return "";
  const effectEntry = (abilityData.effect_entries || []).find(
    (entry) => entry.language?.name === "en"
  );
  if (effectEntry?.short_effect) {
    return effectEntry.short_effect.replace(/\s+/g, " ").trim();
  }
  if (effectEntry?.effect) {
    return effectEntry.effect.replace(/\s+/g, " ").trim();
  }
  return "";
}
...
async function resolveAbilities(pokemonData) {
  ...
  const abilityEntries = Array.isArray(pokemonData.abilities)
    ? pokemonData.abilities
    : [];
  const results = await Promise.all(
    abilityEntries.map(async (entry) => {
      if (!entry?.ability?.name) return null;
      const abilityData = await fetchAbilityResource(entry.ability.name);
      return {
        name: formatAbilityName(entry.ability.name),
        hidden: Boolean(entry.is_hidden),
        effect: extractAbilityEffect(abilityData),
      };
    })
  );
...
function extractCategory(speciesData) {
  if (!speciesData?.genera) return "";
  const match = speciesData.genera.find((genus) => genus.language?.name === "en");
  return match?.genus ? match.genus.replace(/ Pokémon$/i, " Pokémon") : "";
}
...
function extractPokedexEntries(speciesData) {
  ...
  speciesData.flavor_text_entries.forEach((entry) => {
    ...
    if (!text) return;
    if (!entries.has(key)) {
      entries.set(key, { version: key, versionName: formatVersionName(version), text });
    }
  });
  return Array.from(entries.values());
}

function mapStatSlugToLabel(slug) {
  switch (slug) {
    case "hp":
      return "HP";
    case "attack":
    ...
    case "special-defense":
      return "Sp. Verdediging";
    case "speed":
      return "Snelheid";
    default:
      return toTitleCase(slug || "Stat");
  }
}

function buildStatSummary(pokemonData) {
  if (!pokemonData?.stats) return [];
  return pokemonData.stats
    .map((stat) => {
      const slug = stat.stat?.name || "";
      const value = Number.isFinite(stat.base_stat) ? stat.base_stat : null;
      return value === null
        ? null
        : {
            slug,
            label: mapStatSlugToLabel(slug),
            value,
          };
    })
    .filter(Boolean);
...
function extractSpriteFromPokemon(pokemonData, key) {
  if (!pokemonData?.sprites) return null;
  if (key === "female") {
    return pokemonData.sprites.front_female || null;
  }
  if (key === "shinyFemale") {
    return pokemonData.sprites.front_shiny_female || null;
  }
  return null;
}

async function fetchSpeciesSpritesMap(chainData, baseSpeciesId, baseSprites) {
  const spriteMap = new Map();
  if (baseSpeciesId && baseSprites) {
    spriteMap.set(baseSpeciesId, baseSprites.home || baseSprites.default || baseSprites.artwork || null);
  }
  ...
  function collect(node) {
    ...
    if (id && id !== baseSpeciesId) {
      ids.add(id);
    }
    (node.evolves_to || []).forEach(collect);
  }

  collect(chainData.chain);

  const promises = Array.from(ids.values()).map(async (id) => {
    const data = await fetchPokemonResource(id);
    if (!data) return;
    const sprite =
      data.sprites?.other?.home?.front_default ||
...
function buildEvolutionSteps(chainData, targetSpeciesId, spriteMap = new Map()) {
  if (!chainData?.chain) {
    return { steps: [] };
  }
  const steps = [];
  ...
  function traverse(node, parent) {
    ...
    const sprite = spriteMap.get(speciesId) || null;
    const currentNode = { speciesId, speciesName, sprite };
    ...
    if (parent) {
      ...
      steps.push({
        from: {
          ...
          sprite: spriteMap.get(parent.speciesId) || null,
        },
        to: {
          id: speciesId,
          name: speciesName,
...
async function getPokemonDetails(entry) {
  ...
  const abilities = await resolveAbilities(pokemonData);
  const category = extractCategory(speciesData);
  const stats = buildStatSummary(pokemonData);
  ...
  const femaleSprite = extractSpriteFromPokemon(pokemonData, "female");
  const shinyFemaleSprite = extractSpriteFromPokemon(pokemonData, "shinyFemale");
  const sprites = {
  ...
  if (speciesData?.evolution_chain?.url) {
    ...
    spriteMap = await fetchSpeciesSpritesMap(chainData, entry.speciesId, sprites);
    evolution = buildEvolutionSteps(chainData, entry.speciesId, spriteMap);
  }

  const pokedexEntries = extractPokedexEntries(speciesData);
