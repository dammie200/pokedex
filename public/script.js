...
function updateViewToggleButtons() {
  ...
  if (dom.viewListToggle) {
    dom.viewListToggle.classList.toggle("is-active", !isGrid);
    dom.viewListToggle.setAttribute("aria-pressed", String(!isGrid));
  }
  if (dom.dexGrid) {
    dom.dexGrid.hidden = !isGrid;
  }
  if (dom.dexList) {
    dom.dexList.hidden = isGrid;
  }
}
...
function updateBoxIndicator(current, total) {
  if (dom.boxIndicator) {
    dom.boxIndicator.textContent = isGridView()
      ? `Box ${current + 1} / ${total}`
      : "Lijstweergave";
  }
  if (dom.boxInput) {
